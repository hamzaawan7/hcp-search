...
const Search = () => {
  ...
  const handleSearch = async (page = 1, limit = 10) => {
    ...
    try {
      ...
      if (searchType === "smart") {
        const exactResponse = await axios.get("http://localhost:5001/search/smart/exact", { params });
        setExact(exactResponse.data.results || []);
        setExactPagination(exactResponse.data.pagination || {});
        const suggestedResponse = await axios.get("http://localhost:5001/search/smart/suggested", { params });
        ...
        const response = await axios.get("http://localhost:5001/search/direct", { params });
        setResults(response.data.results || []);
        setPagination(response.data.pagination || {});
        setExact([]);
        setSuggested([]);
      } else if (searchType === "multiple") {
        const response = await axios.get("http://localhost:5001/search/multiple", { params });
        setResults(response.data.results || []);
        setPagination(response.data.pagination || {});
        setExact([]);
        setSuggested([]);
